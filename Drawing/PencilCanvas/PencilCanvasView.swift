...
final class MacCanvasNSView: NSView {
    ...
    private func shapePoints(for tool: MacDrawingTool) -> [MacPoint] {
        ...
        case .circle:
            ...
            let radiusX = abs(b.x - a.x) / 2
            let radiusY = abs(b.y - a.y) / 2
            return (0...32).map { step in
                let angle = CGFloat(step) * 2 * .pi / 32
                return MacPoint(CGPoint(x: center.x + radiusX * cos(angle), y: center.y + radiusY * sin(angle)))
            }
        }
    }
