...
function NotebookSurface({ document, update, status, error, conflict, copying, save, keepBoth }: Omit<ReturnType<typeof useNotebook>, "document"> & { document: NotebookDocument }) {
  ...
  return <div className={styles.editor} inert={copying} aria-busy={copying} onKeyDown={event => {
    ...
    <header className={styles.toolbar}>
      <div className={styles.title}><h1 className="sr-only">Notebook editor</h1><input aria-label="Notebook title" value={title ?? document.title} maxLength={120} onChange={event => { setTitle(event.target.value); if (event.target.value.trim()) commit({ ...document, title: event.target.value.trim() }); }} onBlur={() => setTitle(null)} /><span role="status">{status}</span></div>
      <div className={styles.actions}><button type="button" aria-label="Undo" title="Undo (⌘Z)" disabled={!history.past.length} onClick={undo}><Icon name="undo" width="17" /></button><button type="button" aria-label="Redo" title="Redo (⇧⌘Z)" disabled={!history.future.length} onClick={redo}><Icon name="redo" width="17" /></button><button type="button" aria-label="Previous page" disabled={currentPage === 0} onClick={() => goTo(currentPage - 1)}>‹</button><select aria-label="Current page" value={Math.min(currentPage, document.pages.length - 1)} onChange={event => goTo(Number(event.target.value))}>{document.pages.map((item, index) => <option key={item.id} value={index}>Page {index + 1}</option>)}</select><button type="button" aria-label="Next page" disabled={currentPage >= document.pages.length - 1} onClick={() => goTo(currentPage + 1)}>›</button><button type="button" aria-label="Add page" title="Add page" disabled={document.pages.length >= 300} onClick={addPage}><Icon name="plus" width="18" /></button><button type="button" aria-expanded={settings} onClick={() => setSettings(!settings)}>Paper & page</button><details className={styles.exportMenu}><summary aria-label="Export notebook"><Icon name="export" width="17" /></summary><div><button type="button" disabled={exporting} onClick={() => void pdf()}>Export PDF</button><button type="button" disabled={exporting} onClick={() => void pdf(true)}>Current page as PDF</button><button type="button" onClick={() => exportDocument(document)}>Export JSON</button><button type="button" onClick={() => void save()}>Save now</button></div></details></div>
    </header>
    {settings && <div className={styles.settings}><label>Paper<select aria-label="Paper template" value={paper.template} onChange={event => page.inheritsStyle ? commit({ ...document, template: event.target.value as Template }) : changePage(page.id, { template: event.target.value as Template })}>{templates.map(value => <option key={value}>{value}</option>)}</select></label><label>Color<select aria-label="Paper color" value={paper.color} onChange={event => page.inheritsStyle ? commit({ ...document, color: event.target.value as PaperColor }) : changePage(page.id, { color: event.target.value as PaperColor })}>{paperColors.map(value => <option key={value}>{value}</option>)}</select></label><label>Size<select aria-label="Page size" value={page.size} onChange={event => changePage(page.id, { size: event.target.value as PageSize })}>{pageSizes.map(value => <option key={value} value={value}>{value === "letterPortrait" ? "US Letter" : "A4"}</option>)}</select></label><label><input type="checkbox" checked={page.inheritsStyle} onChange={event => changePage(page.id, { inheritsStyle: event.target.checked, ...paper })} />Use notebook style</label><button type="button" disabled={document.pages.length === 1} onClick={() => { commit({ ...document, pages: document.pages.filter(item => item.id !== page.id) }); goTo(Math.max(0, currentPage - 1)); select({ pageId: "", ids: [] }); }}>Delete page</button></div>}
    ...
    <div className={styles.canvasArea} onWheelCapture={event => {
      ...
      <FloatingTools tool={tool} choose={choose} color={color} setColor={setColor} width={width} setWidth={setWidth} opacity={opacity} setOpacity={setOpacity} canDelete={selection.ids.length > 0} remove={removeSelection} />
