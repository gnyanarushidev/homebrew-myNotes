...
import { accountHome, createRequestAuth, isAllowedAccount, PRIVATE_HEADERS } from "@/server/auth/client";
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!result || result.error || !isAllowedAccount(data.user, settings)) {
      auth.clearSession();
      return fail();
    }
    const next = type === "invite" || type === "recovery" || authDestination(params.get("next")) === "/reset-password" ? "/reset-password" : accountHome(data.user, settings);
